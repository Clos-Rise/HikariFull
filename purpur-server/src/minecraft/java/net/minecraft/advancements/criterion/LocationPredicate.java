...
public record LocationPredicate(
    ...
    public boolean matches(final ServerLevel level, final double x, final double y, final double z) {
        ...
        if (this.dimension.isPresent() && this.dimension.get() != (io.papermc.paper.configuration.GlobalConfiguration.get().misc.strictAdvancementDimensionCheck ? level.dimension() : org.bukkit.craftbukkit.util.CraftDimensionUtil.getMainDimensionKey(level))) { // Paper - Add option for strict advancement dimension checks
            return false;
        }

        BlockPos pos = BlockPos.containing(x, y, z);
        boolean loaded = level.isLoaded(pos);
        return (!this.biomes.isPresent() || loaded && this.biomes.get().contains(level.getBiome(pos)))
            && (!this.structures.isPresent() || loaded && level.structureManager().getStructureWithPieceAt(pos, this.structures.get()).isValid())
            && (!this.smokey.isPresent() || loaded && this.smokey.get() == CampfireBlock.isSmokeyPos(level, pos))
            && (!this.light.isPresent() || this.light.get().matches(level, pos))
            && (!this.block.isPresent() || this.block.get().matches(level, pos))
            && (!this.fluid.isPresent() || this.fluid.get().matches(level, pos))
