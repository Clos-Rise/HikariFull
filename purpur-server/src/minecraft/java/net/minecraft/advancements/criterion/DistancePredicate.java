...
public record DistancePredicate(
    ...
    public boolean matches(final double x0, final double y0, final double z0, final double x1, final double y1, final double z1) {
        ...
        return this.x.matches(Mth.abs(xd))
            && this.y.matches(Mth.abs(yd))
            && this.z.matches(Mth.abs(zd))
            && this.horizontal.matchesSqr(xd * xd + zd * zd)
            && this.absolute.matchesSqr(xd * xd + yd * yd + zd * zd);
    }
}
