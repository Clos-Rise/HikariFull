...
public record CollectionPredicate<T, P extends Predicate<T>>(
    ...
    public boolean test(final Iterable<? extends T> value) {
        return (!this.contains.isPresent() || this.contains.get().test(value))
            && (!this.counts.isPresent() || this.counts.get().test(value))
            && (!this.size.isPresent() || this.size.get().matches(Iterables.size(value)));
    }
}
