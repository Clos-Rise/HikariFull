...
public class Ravager extends Raider {
    ...
    private static final Predicate<Entity> ROAR_TARGET_WITHOUT_GRIEFING = entity -> ROAR_TARGET_WITH_GRIEFING.test(entity)
        && !entity.is(EntityType.ARMOR_STAND);
    ...
    private void stunEffect() {
        if (this.random.nextInt(6) == 0) {
            double headX = this.getX() - this.getBbWidth() * Math.sin(this.yBodyRot * Mth.DEG_TO_RAD) + (this.random.nextDouble() * 0.6 - 0.3);
            double headY = this.getY() + this.getBbHeight() - 0.3;
            double headZ = this.getZ() + this.getBbWidth() * Math.cos(this.yBodyRot * Mth.DEG_TO_RAD) + (this.random.nextDouble() * 0.6 - 0.3);
            this.level()
    ...
    public boolean hasLineOfSight(final Entity target) {
        return this.stunnedTick <= 0 && this.roarTick <= 0 && super.hasLineOfSight(target);
    }
    ...
    private void roar() {
        if (this.isAlive() && this.level() instanceof ServerLevel level) {
            ...
            level.broadcastEntityEvent(this, EntityEvent.RAVAGER_ROARED);
        }
    }
    ...
    private void strongKnockback(final Entity entity) {
        ...
        entity.push(xd / dd * 4.0, 0.2, zd / dd * 4.0, this); // Paper - Add EntityKnockbackByEntityEvent and EntityPushedByEntityAttackEvent
    }
