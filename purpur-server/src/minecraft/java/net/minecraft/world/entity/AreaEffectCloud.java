    ...
    private void clientTick() {
        ...
        if (!isWaiting || !this.random.nextBoolean()) {
            ...
            if (isWaiting) {
                ...
                particleCount = Mth.ceil(Mth.PI * radius * radius);
                    ...
                    if (!this.potionContents.hasEffects()) {
                        ...
                        if (!entities.isEmpty()) {
                            ...
                            if (!event.isCancelled()) {
                                for (org.bukkit.entity.LivingEntity bukkitLivingEntity : event.getAffectedEntities()) {
                                    if (bukkitLivingEntity instanceof org.bukkit.craftbukkit.entity.CraftLivingEntity craftLivingEntity) {
                                        ...
                                        for (MobEffectInstance effect : allEffects) {
                                            if (effect.getEffect().value().isInstantenous()) {
                                                effect.getEffect()
                                                    .value()
                                                    .applyInstantenousEffect(serverLevel, this, this.getOwner(), entity, effect.getAmplifier(), 0.5);
                                            } else {
