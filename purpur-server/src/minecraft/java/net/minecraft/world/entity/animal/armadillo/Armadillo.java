...
public class Armadillo extends Animal {
    ...
    public boolean isScaredBy(final LivingEntity livingEntity) {
        return this.getBoundingBox().inflate(7.0, 2.0, 7.0).intersects(livingEntity.getBoundingBox())
            && (
                livingEntity.is(EntityTypeTags.UNDEAD)
                    || this.getLastHurtByMob() == livingEntity
                    || livingEntity instanceof Player player && !player.isSpectator() && (player.isSprinting() || player.isPassenger())
            );
    }
    ...
    public boolean actuallyHurt(final ServerLevel level, final DamageSource source, final float dmg, org.bukkit.event.entity.EntityDamageEvent event) {
