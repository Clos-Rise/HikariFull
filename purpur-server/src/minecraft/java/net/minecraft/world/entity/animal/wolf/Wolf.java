...
public class Wolf extends TamableAnimal implements NeutralMob {
    ...
    public void setVariant(final Holder<WolfVariant> variant) {
    ...
    public void setSoundVariant(final Holder<WolfSoundVariant> soundVariant) {
    ...
    protected void addAdditionalSaveData(final ValueOutput output) {
        ...
        this.getSoundVariant()
            .unwrapKey()
            .ifPresent(
                soundVariant -> output.store("sound_variant", ResourceKey.codec(Registries.WOLF_SOUND_VARIANT), (ResourceKey<WolfSoundVariant>)soundVariant)
            );
    }
    ...
    public float getHeadRollAngle(final float a) {
        return Mth.lerp(a, this.interestedAngleO, this.interestedAngle) * 0.15F * Mth.PI;
    }
    ...
    public boolean hurtServer(final ServerLevel level, final DamageSource source, final float damage) {
        ...
        if (!super.hurtServer(level, source, damage)) return false; // CraftBukkit
    ...
    @Override
    public boolean actuallyHurt(final ServerLevel level, final DamageSource source, final float damage, final org.bukkit.event.entity.EntityDamageEvent event) { // CraftBukkit - void -> boolean
        if (!this.canArmorAbsorb(source)) {
            return super.actuallyHurt(level, source, damage, event); // CraftBukkit
        } else {
    ...
    public float getTailAngle() {
        if (this.isAngry()) {
            ...
            return (0.55F - damageRatio * 0.4F) * Mth.PI;
        } else {
    ...
    public boolean wantsToAttack(final LivingEntity target, final LivingEntity owner) {
        if (target instanceof Creeper || target instanceof Ghast || target instanceof ArmorStand) {
            ...
            return target instanceof Wolf wolfTarget
                ? !wolfTarget.isTame() || wolfTarget.getOwner() != owner
                : !(target instanceof Player playerTarget && owner instanceof Player playerOwner && !playerOwner.canHarmPlayer(playerTarget))
                    && !(target instanceof AbstractHorse horse && horse.isTamed())
                    && !(target instanceof TamableAnimal animal && animal.isTame());
        }
    }
