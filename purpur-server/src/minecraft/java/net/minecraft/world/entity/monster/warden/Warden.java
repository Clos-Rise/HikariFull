...
public class Warden extends Monster implements VibrationSystem {
    ...
    public boolean isInvulnerableTo(final ServerLevel level, final DamageSource source) {
        return this.isDiggingOrEmerging() && !source.is(DamageTypeTags.BYPASSES_INVULNERABILITY) || super.isInvulnerableTo(level, source);
    }
    ...
    public boolean doHurtTarget(final ServerLevel level, final Entity target) {
        ...
        SonicBoom.setCooldown(this, SonicBoom.COOLDOWN);
    ...
    public static void applyDarknessAround(final ServerLevel level, final Vec3 position, final @Nullable Entity source, final int darknessRadius) {
        ...
        MobEffectUtil.addEffectToPlayersAround(level, source, position, darknessRadius, darkness, 200, org.bukkit.event.entity.EntityPotionEffectEvent.Cause.WARDEN); // CraftBukkit - Add EntityPotionEffectEvent.Cause
    }
    ...
    @VisibleForTesting
    public void increaseAngerAt(final @Nullable Entity entity, int amount, final boolean playSound) { // Paper - Add WardenAngerChangeEvent
