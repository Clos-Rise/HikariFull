...
public class RamTarget extends Behavior<Goat> {
    ...
    public RamTarget(
        ...
        super(
            ImmutableMap.of(MemoryModuleType.RAM_COOLDOWN_TICKS, MemoryStatus.VALUE_ABSENT, MemoryModuleType.RAM_TARGET, MemoryStatus.VALUE_PRESENT),
            TIME_OUT_DURATION
        );
    ...
    protected void tick(final ServerLevel level, final Goat body, final long timestamp) {
        ...
        if (!nearbyEntities.isEmpty()) {
            ...
            float speedFactor = Mth.clamp(body.getSpeed() * RAM_SPEED_FORCE_FACTOR, 0.2F, 3.0F) + speedBoostPower;
            ...
            ramTarget.knockback(blockingFactor * speedFactor * this.getKnockbackForce.applyAsDouble(body), this.ramDirection.x(), this.ramDirection.z(), body, io.papermc.paper.event.entity.EntityKnockbackEvent.Cause.ENTITY_ATTACK); // Paper - Add EntityKnockbackByEntityEvent and EntityPushedByEntityAttackEvent
    ...
    protected void finishRam(final ServerLevel level, final Goat body) {
        level.broadcastEntityEvent(body, EntityEvent.END_RAM);
