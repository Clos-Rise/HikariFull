...
public class HappyGhast extends Animal {
    ...
    public boolean canBeCollidedWith(final @Nullable Entity other) {
        return !this.isBaby()
            && this.isAlive()
            && (
                this.level().isClientSide() && other instanceof Player && other.position().y >= this.getBoundingBox().maxY
                    || this.isVehicle() && other instanceof HappyGhast
    ...
    private class HappyGhastLookControl extends LookControl {
        ...
        public void tick() {
            if (HappyGhast.this.isOnStillTimeout()) {
                ...
                HappyGhast.this.setYRot(-((float)Mth.atan2(xdd, zdd)) * Mth.RAD_TO_DEG);
