...
public class DripstoneClusterFeature extends Feature<DripstoneClusterConfiguration> {
    ...
    private void placeColumn(
        ...
        final float density,
        final DripstoneClusterConfiguration config
    ) {
        ...
        if (!baseColumn.isEmpty()) {
            ...
            if (!ceiling.isEmpty() || !baseFloor.isEmpty()) {
                ...
                if (wantPool && baseFloor.isPresent() && this.canPlacePool(level, pos.atY(baseFloor.getAsInt()))) {
                    ...
                    level.setBlock(pos.atY(baseFloorY), Blocks.WATER.defaultBlockState(), Block.UPDATE_CLIENTS);
                } else {
    ...
    private int getDripstoneHeight(
        final RandomSource random, final int dx, final int dz, final float density, final int maxHeight, final DripstoneClusterConfiguration config
    ) {
    ...
    private double getChanceOfStalagmiteOrStalactite(
        final int xRadius, final int zRadius, final int dx, final int dz, final DripstoneClusterConfiguration config
    ) {
        ...
        return Mth.clampedMap(
            distanceFromEdge, 0.0F, config.maxDistanceFromEdgeAffectingChanceOfDripstoneColumn, config.chanceOfDripstoneColumnAtMaxDistanceFromCenter, 1.0F
        );
    }
