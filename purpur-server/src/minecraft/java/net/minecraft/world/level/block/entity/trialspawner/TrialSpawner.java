...
public final class TrialSpawner {
    ...
    public void applyOminous(final ServerLevel level, final BlockPos spawnerPos) {
        level.setBlock(spawnerPos, level.getBlockState(spawnerPos).setValue(TrialSpawnerBlock.OMINOUS, true), Block.UPDATE_ALL);
        level.levelEvent(LevelEvent.PARTICLES_TRIAL_SPAWNER_BECOME_OMINOUS, spawnerPos, 1);
        this.isOminous = true;
    ...
    public void removeOminous(final ServerLevel level, final BlockPos spawnerPos) {
        level.setBlock(spawnerPos, level.getBlockState(spawnerPos).setValue(TrialSpawnerBlock.OMINOUS, false), Block.UPDATE_ALL);
    ...
    public Optional<UUID> spawnMob(final ServerLevel level, final BlockPos spawnerPos) {
        ...
        try (ProblemReporter.ScopedCollector reporter = new ProblemReporter.ScopedCollector(() -> "spawner@" + spawnerPos, LOGGER)) {
            ...
            if (org.bukkit.craftbukkit.event.CraftEventFactory.callTrialSpawnerSpawnEvent(entity, spawnerPos).isCancelled()) {
                return Optional.empty();
            }
            ...
            level.levelEvent(LevelEvent.PARTICLES_TRIAL_SPAWNER_SPAWN, spawnerPos, flameParticle.encode());
            level.levelEvent(LevelEvent.PARTICLES_TRIAL_SPAWNER_SPAWN_MOB_AT, spawnBlockPos, flameParticle.encode());
    ...
    public void ejectReward(final ServerLevel level, final BlockPos pos, final ResourceKey<LootTable> ejectingLootTable) {
        ...
        if (!lootDrops.isEmpty()) {
            ...
            lootDrops = new ObjectArrayList<>(spawnerDispenseLootEvent.getDispensedLoot().stream().map(org.bukkit.craftbukkit.inventory.CraftItemStack::asNMSCopy).toList());
            // CraftBukkit end
