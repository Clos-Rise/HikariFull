...
public class LargeDripstoneFeature extends Feature<LargeDripstoneConfiguration> {
    ...
    public boolean place(final FeaturePlaceContext<LargeDripstoneConfiguration> context) {
        ...
        Optional<Column> column = Column.scan(
            level, origin, config.floorToCeilingSearchRange, DripstoneUtils::isEmptyOrWater, DripstoneUtils::isDripstoneBaseOrLava
        );
        if (!column.isEmpty() && column.get() instanceof Column.Range) {
            ...
            if (stalactite.isSuitableForWind(config) && stalagmite.isSuitableForWind(config)) {
                wind = new LargeDripstoneFeature.WindOffsetter(origin.getY(), random, config.windSpeed);
            } else {
    ...
    private void placeDebugMarkers(final WorldGenLevel level, final BlockPos origin, final Column.Range range, final LargeDripstoneFeature.WindOffsetter wind) {
        level.setBlock(wind.offset(origin.atY(range.ceiling() - 1)), Blocks.DIAMOND_BLOCK.defaultBlockState(), Block.UPDATE_CLIENTS);
        level.setBlock(wind.offset(origin.atY(range.floor() + 1)), Blocks.GOLD_BLOCK.defaultBlockState(), Block.UPDATE_CLIENTS);
        ...
        for (BlockPos.MutableBlockPos pos = origin.atY(range.floor() + 2).mutable(); pos.getY() < range.ceiling() - 1; pos.move(Direction.UP)) {
            ...
            if (DripstoneUtils.isEmptyOrWater(level, windAdjustedPos) || level.getBlockState(windAdjustedPos).is(Blocks.DRIPSTONE_BLOCK)) {
                level.setBlock(windAdjustedPos, Blocks.CREEPER_HEAD.defaultBlockState(), Block.UPDATE_CLIENTS);
            }
        }
    }
                        ...
                        if (height > 0) {
                            ...
                            for (int i = 0; i < height && pos.getY() < maxY; i++) {
                                ...
                                if (DripstoneUtils.isEmptyOrWaterOrLava(level, windAdjustedPos)) {
                                    ...
                                    level.setBlock(windAdjustedPos, block.defaultBlockState(), Block.UPDATE_CLIENTS);
    ...
    private static final class WindOffsetter {
        ...
        private WindOffsetter(final int originY, final RandomSource random, final FloatProvider windSpeedRange) {
            ...
            float direction = Mth.randomBetween(random, 0.0F, Mth.PI);
